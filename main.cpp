 ...
 */
 void performDecomposition(const std::vector<Sample>& samples, const std::vector<uint16_t>& payloads, Json::Value& root){
    ...
    for (int i = 0; i < payloads.size(); i++) {
        ...
        double S = getS(ar, payload_size, G, fr);
...
int main(int argc, char **argv) {
    ...
    app.add_option("--precision", precision, "Numerical precision in the output.")->check(CLI::Range(2, 16));
    ...
    if (cdf_path.empty()) {
        ...
        cdf = make_cdf(cdf_rtts, &cdf_percentiles);
    }
    ...
    if (!qta_path.empty()) {
        ...
        auto qta = make_cdf(qta_rtts, &qta_percentiles);

        root["cdf_qta_overlap"] = cdf_qta_overlap(cdf, qta);
    }


    root["CDF"].setComment("// Normalized CDF consisting of latency-percentile pairs. When plotting, latency is x and percentile [0, 1] is y.", Json::commentBefore);
    root["cdf_qta_overlap"].setComment("// The overlap between the CDF and the provided QTA. >0 if there is an overlap.", Json::commentBefore);
    root["decomposition"]["intercept"].setComment("// The intercept of the slope found by linear regression through the min value of each packet size.", Json::commentBefore);
    root["decomposition"]["slope"].setComment("// The slope found by linear regression through the min value of each packet size.", Json::commentBefore);
    root["decomposition"]["values"].setComment("//Array of G(eographic)-, S(erialization)-, and V(ariable contention)-delays for each packet size. V is a distribution.", Json::commentBefore);
    //------------------ Output ---------------
